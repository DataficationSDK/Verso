...
Images and links written relative to the notebook keep working. A markdown cell that says `![](figs/genome.png)` is asking for the file beside it in your repository, and that is where the page fetches it from, so a notebook that leans on a folder of figures reads the same here as it does at home. The one thing to keep in mind is that those files have to be public too: a figure in a private repository is no more reachable than the notebook would be.

A relative link to another notebook, or to a Markdown or Jupyter file, opens as a page here rather than on the forge, so a folder of notebooks or a tree of documentation can be read straight through. Relative links to anything else open on the forge as before, and a link you wrote as a full address is left exactly as you wrote it. Every page carries a "View source" button, so the file it came from is always one click away.
